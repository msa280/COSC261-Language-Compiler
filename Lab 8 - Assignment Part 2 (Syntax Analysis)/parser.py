...
class Scanner:
    ...
    def get_token(self):
        ...
        if (self.current_char_index < len(self.input_string)) and (token == None):
            self.no_token()
...
class If_Else_AST:
    def __init__(self, condition, then, else_tok):
        self.condition = condition
        self.then = then
        self.else_tok = else_tok
    def __repr__(self):
        return 'if-else ' + repr(self.condition) + ' then ' + repr(self.then) + ' else ' + self.repr(self.else_tok) + ' end'
    def indented(self, level):
        return indent('If-Else', level) + self.condition.indented(level+1) + self.then.indented(level+1) + self.else_tok.indented(level+1)
...
def if_statement():
    ...
    if (scanner.lookahead() == Token.ELSE):
        ...
        return If_Else_AST(condition, then, else_tok)    
    scanner.consume(Token.END)
